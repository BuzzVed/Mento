...
class KernelBuilder:
    ...
    def build_hash_quad_interleaved_ma(
        ...
        If skip_s0=True, S0 MA ops for all 4 vectors are assumed already done
        (e.g., pre-computed during an earlier index-update). The hash starts
        from S1.c1 for the first pair, saving 1 cycle.
        """
    ...
    def _list_schedule_segment(self, segment):
        ...
        # Build dependency graph with full WAR tracking (all readers).
        # RAW/WAW: delta=1 (must be in later cycle).
        # WAR: delta=0 (can share same cycle under deferred writeback).
        preds = [[] for _ in range(n)]
        succs = [[] for _ in range(n)]
        last_write = {}
        all_readers = defaultdict(list)
        edge_set = set()
        ...
        for i, op in enumerate(ops):
            ...
            for reg in op["written"]:
                ...
                if p is not None and (p, i) not in edge_set:
                    ...
                    succs[p].append((i, 1))
            for reg in op["written"]:
                for r in all_readers.get(reg, []):
                    if (r, i) not in edge_set:
                        edge_set.add((r, i))
                        preds[i].append((r, 0))
                        succs[r].append((i, 0))
            for reg in op["used"]:
                all_readers[reg].append(i)
            for reg in op["written"]:
                last_write[reg] = i
                all_readers[reg] = []
        ...
        # Pre-compute in-degree template
        in_deg_base = [0] * n
        for i in range(n):
            seen = set()
            for p, d in preds[i]:
        ...
        # Compute ASAP times via topological forward pass
        topo_order = []
        in_deg_tmp = list(in_deg_base)
        queue = [i for i in range(n) if in_deg_tmp[i] == 0]
        asap = [0] * n
        while queue:
            i = queue.pop()
            topo_order.append(i)
            for s, d in succs[i]:
                asap[s] = max(asap[s], asap[i] + d)
                in_deg_tmp[s] -= 1
                if in_deg_tmp[s] == 0:
                    queue.append(s)

        # Compute ALAP times via backward pass
        max_asap = max(asap)
        alap = [max_asap] * n
        for i in reversed(topo_order):
            for s, d in succs[i]:
                alap[i] = min(alap[i], alap[s] - d)

        # Slack = ALAP - ASAP; low slack = critical = schedule first
        slack = [alap[i] - asap[i] for i in range(n)]
        nw_scaled = 0.04 * (max_asap + 1)

        # Pre-compute engine info for each op
        op_engine = [ops[i]["engine"] for i in range(n)]
        op_limit = [SLOT_LIMITS[ops[i]["engine"]] for i in range(n)]
        ...
        for trial in range(_N_TRIALS):
            seed = 7 * trial * trial + 13 * trial + 1
            rng = _rng_mod.Random(seed)

            in_deg = list(in_deg_base)

            priorities = [slack[i] + rng.random() * nw_scaled for i in range(n)]

            ready = [(priorities[i], i) for i in range(n) if in_deg[i] == 0]
            heapq.heapify(ready)
            ...
            while ready:
                ...
                eng = op_engine[i]
                lim = op_limit[i]
                while cycle_slots[c][eng] >= lim:
                    c += 1
                if c > cur_max:
                    cur_max = c
                    if best_max_cycle is not None and cur_max >= best_max_cycle:
                        aborted = True
                        break
                op_cycle[i] = c
                cycle_slots[c][eng] += 1
            ...
            if not aborted:
                if best_max_cycle is None or cur_max < best_max_cycle:
                    best_max_cycle = cur_max
                    best_cycle_map = list(op_cycle)

        cycle_bundles = defaultdict(dict)
        for i in range(n):
            cycle_bundles[best_cycle_map[i]].setdefault(ops[i]["engine"], []).append(ops[i]["slot"])
    ...
    def build_index_update_quad_no_clamp(
        ...
        if not lsb_precomputed:
            self.add_bundle({"valu": [("&", t0, val0, one_vec), ("&", t1, val1, one_vec), ("&", t2, val2, one_vec), ("&", t3, val3, one_vec)]})
        self.add_bundle({"valu": [('+', t0, t0, one_vec), ('+', t1, t1, one_vec), ('+', t2, t2, one_vec), ('+', t3, t3, one_vec)]})
        ma_ops = [("multiply_add", idx0, idx0, two_vec, t0), ("multiply_add", idx1, idx1, two_vec, t1), ("multiply_add", idx2, idx2, two_vec, t2), ("multiply_add", idx3, idx3, two_vec, t3)]
        if skip_addr and last_step_extra_valu:
            ma_ops.extend(last_step_extra_valu)
        self.add_bundle({"valu": ma_ops})
        if not skip_addr:
    ...
    def build_index_update_quad_no_clamp_with_loads(
        ...
        if not skip_addr:
            steps.append([('+', addr0, idx0, base_vec), ('+', addr1, idx1, base_vec), ('+', addr2, idx2, base_vec), ('+', addr3, idx3, base_vec)])
        for step_i, valu_ops in enumerate(steps):
            full_ops = list(valu_ops)
    ...
    def _emit_quad_group_rounds(self, g, rounds, forest_height,
                                idx_vec, val_vec, node_vec, addr_vec, tmp1_vec, tmp2_vec, tmp3_vec,
                                idx_vec1, val_vec1, node_vec1, addr_vec1, tmp1_vec1, tmp2_vec1, tmp3_vec1,
                                idx_vec2, val_vec2, node_vec2, addr_vec2, tmp1_vec2, tmp2_vec2, tmp3_vec2,
                                idx_vec3, val_vec3, node_vec3, addr_vec3, tmp1_vec3, tmp2_vec3, tmp3_vec3,
                                idx_vec_b, val_vec_b, node_vec_b, addr_vec_b, tmp1_vec_b, tmp2_vec_b, tmp3_vec_b,
                                idx_vec1_b, val_vec1_b, node_vec1_b, addr_vec1_b, tmp1_vec1_b, tmp2_vec1_b, tmp3_vec1_b,
                                idx_vec2_b, val_vec2_b, node_vec2_b, addr_vec2_b, tmp1_vec2_b, tmp2_vec2_b, tmp3_vec2_b,
                                idx_vec3_b, val_vec3_b, node_vec3_b, addr_vec3_b, tmp1_vec3_b, tmp2_vec3_b, tmp3_vec3_b,
                                idx_ptr, val_ptr,
                                idx_ptr1, idx_ptr2, idx_ptr3,
                                val_ptr1, val_ptr2, val_ptr3,
                                idx_ptr_b, idx_ptr1_b, idx_ptr2_b, idx_ptr3_b,
                                val_ptr_b, val_ptr1_b, val_ptr2_b, val_ptr3_b,
                                hash_stage_specs,
                                one_vec, two_vec, zero_vec, n_nodes_vec, base_vec,
                                root_node_vec,
                                use_small_depth_1, use_small_depth_2,
                                base1_vec, base3_vec,
                                lvl1_node1_vec, lvl1_diff_vec,
                                lvl2_node3_vec, lvl2_node5_vec,
                                lvl2_diff_34_vec, lvl2_diff_56_vec,
                                vlen_const, vlen2_const, vlen3_const,
                                vlen4_const, vlen5_const, vlen6_const, vlen7_const,
                                vlen8_const):
        """Emit pointer setup, initial loads, all rounds, and stores for one quad-pair group."""
        # --- Pointer setup ---
        self.add_bundle(
            {
                "alu": [
        ...
        for round_i in range(rounds):
            ...
            # Index A — skip addr when it won't be used
            skip_addr = (next_a_uses_root or next_a_uses_small) or (round_i == rounds - 1)
            if load_pairs_b:
                if is_reset_update_round:
                    # At the reset round (depth = h), all child indices exceed
                    # n_nodes so clamped update gives idx=0 for all.  Reset directly.
                    bundle = {"valu": [
                        ('*', idx_vec, idx_vec, zero_vec),
                        ('*', idx_vec1, idx_vec1, zero_vec),
                        ('*', idx_vec2, idx_vec2, zero_vec),
                        ('*', idx_vec3, idx_vec3, zero_vec),
                        ("^", val_vec_b, val_vec_b, node_vec_b),
                        ("^", val_vec1_b, val_vec1_b, node_vec1_b),
                    ]}
                    if load_pairs_b:
                        bundle["load"] = load_pairs_b[:2]
                        del load_pairs_b[:2]
                    self.add_bundle(bundle)
                else:
            ...
            if round_i != rounds - 1:
                ...
                # Skip last-round Index B: test only checks val, not idx
    ...
    def build_kernel(self, forest_height: int, n_nodes: int, batch_size: int, rounds: int):
        ...
        # --- Second register set for 2-group parallel processing ---
        g1_idx_vec = self.alloc_vec("g1_idx_vec")
        g1_val_vec = self.alloc_vec("g1_val_vec")
        g1_node_vec = self.alloc_vec("g1_node_vec")
        g1_addr_vec = self.alloc_vec("g1_addr_vec")
        g1_tmp1_vec = self.alloc_vec("g1_tmp1_vec")
        g1_tmp2_vec = self.alloc_vec("g1_tmp2_vec")
        g1_tmp3_vec = self.alloc_vec("g1_tmp3_vec")
        g1_idx_vec1 = self.alloc_vec("g1_idx_vec1")
        g1_val_vec1 = self.alloc_vec("g1_val_vec1")
        g1_node_vec1 = self.alloc_vec("g1_node_vec1")
        g1_addr_vec1 = self.alloc_vec("g1_addr_vec1")
        g1_tmp1_vec1 = self.alloc_vec("g1_tmp1_vec1")
        g1_tmp2_vec1 = self.alloc_vec("g1_tmp2_vec1")
        g1_tmp3_vec1 = self.alloc_vec("g1_tmp3_vec1")
        g1_idx_vec2 = self.alloc_vec("g1_idx_vec2")
        g1_val_vec2 = self.alloc_vec("g1_val_vec2")
        g1_node_vec2 = self.alloc_vec("g1_node_vec2")
        g1_addr_vec2 = self.alloc_vec("g1_addr_vec2")
        g1_tmp1_vec2 = self.alloc_vec("g1_tmp1_vec2")
        g1_tmp2_vec2 = self.alloc_vec("g1_tmp2_vec2")
        g1_tmp3_vec2 = self.alloc_vec("g1_tmp3_vec2")
        g1_idx_vec3 = self.alloc_vec("g1_idx_vec3")
        g1_val_vec3 = self.alloc_vec("g1_val_vec3")
        g1_node_vec3 = self.alloc_vec("g1_node_vec3")
        g1_addr_vec3 = self.alloc_vec("g1_addr_vec3")
        g1_tmp1_vec3 = self.alloc_vec("g1_tmp1_vec3")
        g1_tmp2_vec3 = self.alloc_vec("g1_tmp2_vec3")
        g1_tmp3_vec3 = self.alloc_vec("g1_tmp3_vec3")
        g1_idx_vec_b = self.alloc_vec("g1_idx_vec_b")
        g1_val_vec_b = self.alloc_vec("g1_val_vec_b")
        g1_node_vec_b = self.alloc_vec("g1_node_vec_b")
        g1_addr_vec_b = self.alloc_vec("g1_addr_vec_b")
        g1_tmp1_vec_b = self.alloc_vec("g1_tmp1_vec_b")
        g1_tmp2_vec_b = self.alloc_vec("g1_tmp2_vec_b")
        g1_tmp3_vec_b = self.alloc_vec("g1_tmp3_vec_b")
        g1_idx_vec1_b = self.alloc_vec("g1_idx_vec1_b")
        g1_val_vec1_b = self.alloc_vec("g1_val_vec1_b")
        g1_node_vec1_b = self.alloc_vec("g1_node_vec1_b")
        g1_addr_vec1_b = self.alloc_vec("g1_addr_vec1_b")
        g1_tmp1_vec1_b = self.alloc_vec("g1_tmp1_vec1_b")
        g1_tmp2_vec1_b = self.alloc_vec("g1_tmp2_vec1_b")
        g1_tmp3_vec1_b = self.alloc_vec("g1_tmp3_vec1_b")
        g1_idx_vec2_b = self.alloc_vec("g1_idx_vec2_b")
        g1_val_vec2_b = self.alloc_vec("g1_val_vec2_b")
        g1_node_vec2_b = self.alloc_vec("g1_node_vec2_b")
        g1_addr_vec2_b = self.alloc_vec("g1_addr_vec2_b")
        g1_tmp1_vec2_b = self.alloc_vec("g1_tmp1_vec2_b")
        g1_tmp2_vec2_b = self.alloc_vec("g1_tmp2_vec2_b")
        g1_tmp3_vec2_b = self.alloc_vec("g1_tmp3_vec2_b")
        g1_idx_vec3_b = self.alloc_vec("g1_idx_vec3_b")
        g1_val_vec3_b = self.alloc_vec("g1_val_vec3_b")
        g1_node_vec3_b = self.alloc_vec("g1_node_vec3_b")
        g1_addr_vec3_b = self.alloc_vec("g1_addr_vec3_b")
        g1_tmp1_vec3_b = self.alloc_vec("g1_tmp1_vec3_b")
        g1_tmp2_vec3_b = self.alloc_vec("g1_tmp2_vec3_b")
        g1_tmp3_vec3_b = self.alloc_vec("g1_tmp3_vec3_b")
        ...
        if batch_size % VLEN == 0:
            ...
            # Second set of scalar pointers for 2-group parallel
            g1_idx_ptr = self.alloc_scratch("g1_idx_ptr")
            g1_val_ptr = self.alloc_scratch("g1_val_ptr")
            g1_idx_ptr1 = self.alloc_scratch("g1_idx_ptr1")
            g1_idx_ptr2 = self.alloc_scratch("g1_idx_ptr2")
            g1_idx_ptr3 = self.alloc_scratch("g1_idx_ptr3")
            g1_idx_ptr_b = self.alloc_scratch("g1_idx_ptr_b")
            g1_idx_ptr1_b = self.alloc_scratch("g1_idx_ptr1_b")
            g1_idx_ptr2_b = self.alloc_scratch("g1_idx_ptr2_b")
            g1_idx_ptr3_b = self.alloc_scratch("g1_idx_ptr3_b")
            g1_val_ptr1 = self.alloc_scratch("g1_val_ptr1")
            g1_val_ptr2 = self.alloc_scratch("g1_val_ptr2")
            g1_val_ptr3 = self.alloc_scratch("g1_val_ptr3")
            g1_val_ptr_b = self.alloc_scratch("g1_val_ptr_b")
            g1_val_ptr1_b = self.alloc_scratch("g1_val_ptr1_b")
            g1_val_ptr2_b = self.alloc_scratch("g1_val_ptr2_b")
            g1_val_ptr3_b = self.alloc_scratch("g1_val_ptr3_b")

            vec_count = batch_size // VLEN
            quad_pair_groups = vec_count // 8
            rem_vecs = vec_count - (quad_pair_groups * 8)

            vlen16_const = self.scratch_const(16 * VLEN)
            shared_args = dict(
                hash_stage_specs=hash_stage_specs,
                one_vec=one_vec, two_vec=two_vec, zero_vec=zero_vec,
                n_nodes_vec=n_nodes_vec, base_vec=base_vec,
                root_node_vec=root_node_vec,
                use_small_depth_1=use_small_depth_1, use_small_depth_2=use_small_depth_2,
                base1_vec=base1_vec, base3_vec=base3_vec,
                lvl1_node1_vec=lvl1_node1_vec, lvl1_diff_vec=lvl1_diff_vec,
                lvl2_node3_vec=lvl2_node3_vec, lvl2_node5_vec=lvl2_node5_vec,
                lvl2_diff_34_vec=lvl2_diff_34_vec, lvl2_diff_56_vec=lvl2_diff_56_vec,
                vlen_const=vlen_const, vlen2_const=vlen2_const, vlen3_const=vlen3_const,
                vlen4_const=vlen4_const, vlen5_const=vlen5_const,
                vlen6_const=vlen6_const, vlen7_const=vlen7_const,
                vlen8_const=vlen8_const,
            )

            g = 0
            while g + 1 < quad_pair_groups:
                # Set base pointers for group 1 (offset by 8*VLEN from group 0)
                self.add_bundle({"alu": [
                    ("+", g1_idx_ptr, idx_ptr, vlen8_const),
                    ("+", g1_val_ptr, val_ptr, vlen8_const),
                ]})

                # Collect ops for group 0 using deferred emission
                self.start_deferred()
                self._emit_quad_group_rounds(
                    g, rounds, forest_height,
                    idx_vec, val_vec, node_vec, addr_vec, tmp1_vec, tmp2_vec, tmp3_vec,
                    idx_vec1, val_vec1, node_vec1, addr_vec1, tmp1_vec1, tmp2_vec1, tmp3_vec1,
                    idx_vec2, val_vec2, node_vec2, addr_vec2, tmp1_vec2, tmp2_vec2, tmp3_vec2,
                    idx_vec3, val_vec3, node_vec3, addr_vec3, tmp1_vec3, tmp2_vec3, tmp3_vec3,
                ...
                g0_bundles = self.end_deferred()

                # Collect ops for group 1 using deferred emission
                self.start_deferred()
                self._emit_quad_group_rounds(
                    g + 1, rounds, forest_height,
                    g1_idx_vec, g1_val_vec, g1_node_vec, g1_addr_vec, g1_tmp1_vec, g1_tmp2_vec, g1_tmp3_vec,
                    g1_idx_vec1, g1_val_vec1, g1_node_vec1, g1_addr_vec1, g1_tmp1_vec1, g1_tmp2_vec1, g1_tmp3_vec1,
                    g1_idx_vec2, g1_val_vec2, g1_node_vec2, g1_addr_vec2, g1_tmp1_vec2, g1_tmp2_vec2, g1_tmp3_vec2,
                    g1_idx_vec3, g1_val_vec3, g1_node_vec3, g1_addr_vec3, g1_tmp1_vec3, g1_tmp2_vec3, g1_tmp3_vec3,
                    g1_idx_vec_b, g1_val_vec_b, g1_node_vec_b, g1_addr_vec_b, g1_tmp1_vec_b, g1_tmp2_vec_b, g1_tmp3_vec_b,
                    g1_idx_vec1_b, g1_val_vec1_b, g1_node_vec1_b, g1_addr_vec1_b, g1_tmp1_vec1_b, g1_tmp2_vec1_b, g1_tmp3_vec1_b,
                    g1_idx_vec2_b, g1_val_vec2_b, g1_node_vec2_b, g1_addr_vec2_b, g1_tmp1_vec2_b, g1_tmp2_vec2_b, g1_tmp3_vec2_b,
                    g1_idx_vec3_b, g1_val_vec3_b, g1_node_vec3_b, g1_addr_vec3_b, g1_tmp1_vec3_b, g1_tmp2_vec3_b, g1_tmp3_vec3_b,
                    g1_idx_ptr, g1_val_ptr,
                    g1_idx_ptr1, g1_idx_ptr2, g1_idx_ptr3,
                    g1_val_ptr1, g1_val_ptr2, g1_val_ptr3,
                    g1_idx_ptr_b, g1_idx_ptr1_b, g1_idx_ptr2_b, g1_idx_ptr3_b,
                    g1_val_ptr_b, g1_val_ptr1_b, g1_val_ptr2_b, g1_val_ptr3_b,
                    **shared_args,
                )
                g1_bundles = self.end_deferred()

                # Emit both groups sequentially; multi-trial scheduler handles ordering
                self.instrs.extend(g0_bundles)
                self.instrs.extend(g1_bundles)

                # Advance base pointers by 2 groups (16 * VLEN)
                self.add_bundle({"alu": [
                    ("+", idx_ptr, idx_ptr, vlen16_const),
                    ("+", val_ptr, val_ptr, vlen16_const),
                ]})
                g += 2

            # Handle remaining single group (if odd number)
            while g < quad_pair_groups:
                self._emit_quad_group_rounds(
                    g, rounds, forest_height,
                    idx_vec, val_vec, node_vec, addr_vec, tmp1_vec, tmp2_vec, tmp3_vec,
                    idx_vec1, val_vec1, node_vec1, addr_vec1, tmp1_vec1, tmp2_vec1, tmp3_vec1,
                ...
                self.add_bundle({"alu": [
                    ("+", idx_ptr, idx_ptr, vlen8_const),
                    ("+", val_ptr, val_ptr, vlen8_const),
                ]})
                g += 1
