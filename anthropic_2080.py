...
class KernelBuilder:
    ...
    def build_hash_quad_interleaved(
        ...
        """Hash 4 vectors in 13 cycles by interleaving two pairs.

        Pair A = (val0, val1), Pair B = (val2, val3)
        Each hash stage has: cycle1 = op1+op3 (4 valu), cycle2 = op2 (2 valu)
        Interleave: A_c1 | A_c2+B_c1 | B_c2+A_c1(next) | ...
        """
    ...
    def build_hash_quad_interleaved_with_loads(
        ...
        """Hash 4 vectors in 13 cycles, overlapping loads."""
        stages = list(const_vecs)
        ...
        while changed:
            ...
            while i < len(self.instrs):
                if i + 1 < len(self.instrs) and self._can_merge_adjacent(self.instrs[i], self.instrs[i + 1]):
                    out = {}
                    for engine in set(self.instrs[i]) | set(self.instrs[i + 1]):
                        out[engine] = list(self.instrs[i].get(engine, [])) + list(self.instrs[i + 1].get(engine, []))
                    merged.append(out)
                    i += 2
                    changed = True
                else:
        ...
        if batch_size % VLEN == 0:
            ...
            for _ in range(quad_pair_groups):
                ...
                self.add_bundle(
                    {
                        ...
                        "valu": [("+", addr_vec1, idx_vec1, base_vec)],
                    }
                )
                self.add_bundle(
                    {
                        "load": [("vload", idx_vec3, idx_ptr3), ("vload", val_vec3, val_ptr3)],
                        "valu": [("+", addr_vec2, idx_vec2, base_vec)],
                    }
                )
                self.add_bundle(
                    {
                        "load": [("vload", idx_vec_b, idx_ptr_b), ("vload", val_vec_b, val_ptr_b)],
                        "valu": [("+", addr_vec3, idx_vec3, base_vec)],
                    }
                )
                self.add_bundle(
                    {
                        "load": [("vload", idx_vec1_b, idx_ptr1_b), ("vload", val_vec1_b, val_ptr1_b)],
                        "valu": [("+", addr_vec_b, idx_vec_b, base_vec)],
                    }
                )
                self.add_bundle(
                    {
                        "load": [("vload", idx_vec2_b, idx_ptr2_b), ("vload", val_vec2_b, val_ptr2_b)],
                        "valu": [("+", addr_vec1_b, idx_vec1_b, base_vec)],
                    }
                )
                self.add_bundle(
                    {
                        "load": [("vload", idx_vec3_b, idx_ptr3_b), ("vload", val_vec3_b, val_ptr3_b)],
                        "valu": [("+", addr_vec2_b, idx_vec2_b, base_vec)],
                    }
                )

                # No initial gather: round 0 uses root broadcasted node value.
                self.add_bundle({"valu": [("+", addr_vec3_b, idx_vec3_b, base_vec)]})
                ...
                for round_i in range(rounds):
                    ...
                    if round_i != rounds - 1:
                        # Build A gathers for next round unless next A round is root.
                        if next_a_uses_root:
                            load_pairs_a = []
                        else:
